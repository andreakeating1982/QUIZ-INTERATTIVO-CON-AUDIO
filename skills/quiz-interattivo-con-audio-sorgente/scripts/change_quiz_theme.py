...
def replace_questions_ts(path: Path, data: list[dict], backup_dir: Path) -> None:
    ...
    # Neutralizza eventuali residui del nome storico nel commento di testa
    # (il simbolo BECQUER_QUESTIONS, senza accento, resta invariato: è un
    # identificatore interno citato da db.ts/routers.ts/storage.ts).
    new = new.replace("Shared Bécquer questions", "Shared questions")
...
def main():
    ...
    for q in questions:
        ...
        if q["correctAnswer"] not in q["options"]:
            bail(f"Domanda #{q['number']}: 'correctAnswer' deve essere una delle 'options'")
        if q["number"] in seen_numbers:
            bail(f"Domanda #{q['number']}: numero duplicato (ogni domanda deve avere un number unico)")
