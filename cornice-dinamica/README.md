...
Questa cartella contiene la **cornice dinamica** del QUIZ INTERATTIVO: un blocco
...
| File | Descrizione |
|---|---|
| `embed-becquer-quiz-dedicata.html` | ⭐ **Versione DEDICATA (~17 KB, v3 impermeabile + anti-loop)**: serve **solo** «Quiz Interattivo» (URL fisso `https://becquer-quiz.easy-peasy.site/`). 🛡️ **IMPERMEABILE**: funziona anche se il blog ha altre cornici simili nella stessa pagina — ogni istanza è un'isola (id univoci con token, scoping DOM, filtro `e.source`, nessuna app può apparire dentro l'iframe di un'altra). 🛡️ **ANTI-LOOP**: niente transizione CSS sull'altezza, debounce 300 ms, clamp 100–15000 px — l'iframe non «si allunga a dismisura» su mobile. Layout: **titolo e pulsanti sulla stessa riga, tutti centrati**. Pulsanti **Schermo intero** e **Ricarica**, **spinner**, **stato online/errore** con Riprova (timeout 15 s), ping periodico + altezza al resize. Font OpenDyslexic via CORS. **La versione consigliata per il blog.** |
...
| `embed-becquer-quiz.html` | **Versione autosufficiente (~125 KB)**: font incorporati in base64. Funziona anche se l'app è offline (i font restano) ed è robusta su qualsiasi piattaforma. |
...
2. Incolla l'intero contenuto di `embed-becquer-quiz-dedicata.html` (⭐ consigliata)
   oppure di `embed-becquer-quiz-lite.html` (minima) o di
   `embed-becquer-quiz.html` (autosufficiente).
...
| Caratteristica | `...-dedicata.html` | `...-lite.html` | `embed-becquer-quiz.html` |
|---|---|---|---|
| Dimensione | ~17 KB (v3 impermeabile + anti-loop) | ~5 KB | ~125 KB |
...
- **Fix anti-loop (cornice, v3)**: anche lato cornice l'altezza viene
  ...
  1. **Niente transizione CSS** sull'altezza dell'iframe (durante
     ...
     auto-amplifica);
  2. **Debounce 300 ms**: l'altezza è applicata solo quando l'app smette di
     inviare per ~300 ms (layout stabilizzato);
  3. **Clamp di sanità**: valori < 100 px o > 15000 px ignorati;
  4. Le vecchie euristiche di "conferma dei salti sospetti" e di congelamento
     dopo 3 crescite sono state rimosse nella v4 (fix 2026-09-04): bloccavano
     le crescite legittime (es. la dashboard docente che si allarga quando si
     apre una classe). Con l'anti-loop alla radice (nessuna transizione CSS +
     `lf-embedded` che disattiva i 100vh) non servono più.
...
1. **URL dell'app (file esistenti)**: nei file HTML cerca `https://becquer-quiz.easy-peasy.site`
...
modalità ad alto contrasto, righello, ascolto) è in `../ACCESSIBILITA.md` e nella
SKILL di manutenzione `skills/becquer-quiz/SKILL.md`.
