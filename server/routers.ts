...
export const appRouter = router({
  ...
  classes: router({
    ...
    /** Advance past a reading pause screen */
    advanceFromReading: publicProcedure
      .input(z.object({ id: z.string() }))
      .mutation(async ({ input }) => {
        return advanceFromReading(input.id);
      }),
  ...
  questions: router({
    ...
    list: publicProcedure.query(() => {
      return BECQUER_QUESTIONS.map(q => ({
        number: q.number,
    ...
    get: publicProcedure
      ...
      .query(async ({ input }) => {
        const q = BECQUER_QUESTIONS.find(q => q.number === input.number);
    ...
    check: publicProcedure
      ...
      .query(async ({ input }) => {
        const q = BECQUER_QUESTIONS.find(q => q.number === input.questionNumber);
        if (!q) throw new Error("Question not found");
        return {
