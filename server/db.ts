...
import { BECQUER_QUESTIONS } from "./questions";
...
export async function startSession(classId: string) {
  if (!db) throw new Error("Database not available");
  const [cls] = await db.update(schema.classes).set({ sessionStarted: true, currentQuestion: 1, readingPauseStage: 1 }).where(eq(schema.classes.id, classId)).returning();
  return cls;
}
...
export async function nextQuestion(classId: string) {
  ...
  const q = cls.currentQuestion || 0;
  const READING_PAUSE_TRIGGERS: Record<number, number> = { 2: 2, 4: 3, 5: 4, 7: 5 };
  if (READING_PAUSE_TRIGGERS[q]) {
    // Enter reading pause instead of advancing
    const [updated] = await db.update(schema.classes).set({ readingPauseStage: READING_PAUSE_TRIGGERS[q] }).where(eq(schema.classes.id, classId)).returning();
    return updated;
  }
  const newQ = Math.min(q + 1, 10);
  const [updated] = await db.update(schema.classes).set({ currentQuestion: newQ }).where(eq(schema.classes.id, classId)).returning();
  return updated;
}

export async function advanceFromReading(classId: string) {
  if (!db) throw new Error("Database not available");
  const [cls] = await db.select().from(schema.classes).where(eq(schema.classes.id, classId));
  if (!cls) throw new Error("Class not found");
  const stage = cls.readingPauseStage || 0;
  const NEXT_QUESTION: Record<number, number> = { 1: 1, 2: 3, 3: 5, 4: 6, 5: 8 };
  const nextQ = NEXT_QUESTION[stage] || cls.currentQuestion + 1;
  const [updated] = await db.update(schema.classes).set({ currentQuestion: nextQ, readingPauseStage: 0 }).where(eq(schema.classes.id, classId)).returning();
  return updated;
}
...
export async function prevQuestion(classId: string) {
  ...
  const [cls] = await db.select().from(schema.classes).where(eq(schema.classes.id, classId));
  if (!cls) throw new Error("Class not found");
  if (cls.readingPauseStage && cls.readingPauseStage > 0) {
    // From a reading pause, go back to the question before it
    const [updated] = await db.update(schema.classes).set({ readingPauseStage: 0 }).where(eq(schema.classes.id, classId)).returning();
    return updated;
  }
...
export async function addStudent(data: { classId: string; name: string }) {
  ...
  // Cerca uno studente esistente della stessa classe con lo stesso nome normalizzato
...
export interface ReportData {
  ...
  classDate: string;
  classCode: string;
  questions: typeof BECQUER_QUESTIONS;
...
export async function getReportData(classId: string): Promise<ReportData> {
  ...
  const questions = BECQUER_QUESTIONS;
